defmodule NervesHubWeb.Components.DeploymentPage.Activity do
  ...
  defp logs_and_pager_assigns(socket, page_number \\ 1, page_size \\ 25) do
    {logs, audit_pager} =
      AuditLogs.logs_for_feed(socket.assigns.deployment, %{
        page: page_number,
        page_size: page_size
      })
        ...
        <div class="w-full flex flex-col bg-zinc-900 border border-zinc-700 rounded">
          <div class="flex justify-between items-center h-14 px-4 border-b border-zinc-700">
            ...
            <div class="p-1.5 rounded bg-zinc-800 border border-zinc-600">
              <.link href={~p"/org/#{@org.name}/#{@product.name}/deployments/#{@deployment.name}/audit_logs/download"}>
  ...
  def handle_event("set-paginate-opts", %{"page-size" => page_size}, socket) do
    ...
    url =
      ~p"/org/#{socket.assigns.org.name}/#{socket.assigns.product.name}/devices/#{socket.assigns.deployment.name}/activity?#{params}"
