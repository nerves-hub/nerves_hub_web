defmodule NervesHubWeb.Components.Navigation do
  ...
  def updated_sidebar(assigns) do
    ...
    <nav class="flex flex-1 flex-col">
      <ul role="list" class="flex flex-1 flex-col">
        <li>
          <ul role="list">
            <.nav_link label="Devices" path={~p"/org/#{@org.name}/#{@product.name}/devices"} current={@current}>
              <path
            ...
            <.nav_link label="Firmware" path={~p"/org/#{@org.name}/#{@product.name}/firmware"} current={@current}>
              <path
            ...
            <.nav_link label="Artifacts" path={~p"/org/#{@org.name}/#{@product.name}/archives"} current={@current}>
              <path
            ...
            <.nav_link label="Support Scripts" path={~p"/org/#{@org.name}/#{@product.name}/scripts"} current={@current}>
              <path
            ...
            <.nav_link label="Settings" path={~p"/org/#{@org.name}/#{@product.name}/settings"} current={@current}>
              <path
  ...
  defp active_icon(path, current) do
    if path == current do
      " stroke-[#6366F1]"
    else
      " stroke-[#71717A]"
  ...
  def nav_link(assigns) do
    ~H"""
    <li class={"h-11 flex items-center py-2 px-4 " <> if @path == @current do "sidebar-item-selected" else "" end}>
      <.link class="group flex items-center gap-x-3 text-sm tracking-wide leading-[19px] text-[#D4D4D8] font-light w-full h-full" navigate={@path}>
        <svg class={"w-5 h-5" <> active_icon(@path, @current)} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <%= render_slot(@inner_block) %>
        </svg>
        <span class="pt-0.5"><%= @label %></span>
      </.link>
    </li>
    """
  end
