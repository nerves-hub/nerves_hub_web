  ...
  def requires_permission(permission, body, %{args: [_event, _params, socket]}) do
    subject =
      if org_level_permission?(permission) do
        quote(do: unquote(socket).assigns.org)
      else
        quote(do: unquote(socket).assigns.product)
      end

    quote(location: :keep) do
      %{private: private} = socket = RequireAuthorization.authorize!(unquote(socket), unquote(permission), unquote(subject))
      {reply, socket} = unquote(body)
