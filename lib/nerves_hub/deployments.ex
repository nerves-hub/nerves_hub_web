defmodule NervesHub.Deployments do
  ...
  def update_deployment(deployment, params) do
    ...
    case Repo.update(changeset) do
      {:ok, deployment} ->
        ...
        # if the conditions changed, we should  tell any connected devices
  ...
  Do nothing if a deployment is already set and it matches the attached deployment
  """
  ...
  def set_deployment(device) do
    # if the device matches the deployment it's attached to, do nothing
    # otherwise blank it and re-evaluate deployments
    device = Repo.preload(device, [:deployment])

    tags_match? =
      Enum.all?(device.deployment.conditions["tags"], fn tag ->
        Enum.member?(device.tags, tag)
      end)

    if version_match?(device, device.deployment) && tags_match? do
      device
    else
      device
      |> Ecto.Changeset.change()
