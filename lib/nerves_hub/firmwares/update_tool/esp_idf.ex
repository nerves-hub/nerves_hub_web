defmodule NervesHub.Firmwares.UpdateTool.EspIdf do
  ...
  Every ESP-IDF image carries an `esp_app_desc_t` at a **fixed offset of 0x20**,
  immediately after `esp_image_header_t` (24 bytes) and the first
  `esp_image_segment_header_t` (8 bytes). It is 256 bytes and holds the project
  name, version, IDF version and a SHA-256 of the source ELF. The image header
  before it carries the chip ID. Between them they supply every field NervesHub
  requires:
  ...
  A firmware author has nothing to add to their build beyond setting
  `PROJECT_VER` to something NervesHub can read as a version.
  ...
  a leading `v`, two-part `1.2`, and the four-part `1.2.3.4` that ESP tooling
  encourages. Anything else is rejected with a message pointing at
  `PROJECT_VER`, rather than coerced into a version that would sort wrongly.

  ## Signatures

  A signed image carries a Secure Boot v2 signature block in a 4 KB sector
  appended to the image. `verify_signature/2` checks it against the
  organization's registered RSA-3072 keys; see `NervesHub.Accounts.OrgKey`.
