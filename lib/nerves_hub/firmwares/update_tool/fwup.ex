defmodule NervesHub.Firmwares.UpdateTool.Fwup do
  ...
  @impl NervesHub.Firmwares.UpdateTool
  def device_update_type(device, %Firmware{} = target) do
    ...
    with {:ok, %{tool_metadata: tm}} <- delta_result,
         ...
         true <- version_acceptable?(delta_version, device_fwup_version) do
      :delta
    else
      _ ->
        :full
  ...
  defp extract_meta_conf_locally(filepath) do
    try do
      ...
      e ->
        Logger.error("Extracting meta.conf failed due to: #{inspect(e)}")
