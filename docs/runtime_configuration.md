...
| `WEB_BEHIND_TRUSTED_PROXY` | `false` | Whether something in front of this endpoint really does overwrite `WEB_FORWARDED_IP_HEADER`. Separate from naming the header because the two carry different risk: an address that is merely recorded can be wrong without consequence, while the API rate limiter decides on one, and a caller that can forge its own address can pick its own bucket. Until this is set, `NervesHubWeb.Plugs.Attack` throttles by the socket's peer, which behind a balancer is one shared bucket for every caller. Raises at boot if set while the header is `none`. |
