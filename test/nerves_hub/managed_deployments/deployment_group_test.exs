defmodule NervesHub.ManagedDeployments.DeploymentGroupTest do
  ...
  describe "create_changeset/2" do
    setup %{tmp_dir: tmp_dir} do
      ...
      %{deployment_group_params: deployment_group_params, product: product, user: user}
    end
    ...
    test "conditions are required", %{
      ...
      changeset =
        ...
        |> DeploymentGroup.create_changeset(product, user)

      refute changeset.valid?
      refute Enum.empty?(errors_on(changeset).conditions)
    end

    test "tags can be empty", %{deployment_group_params: deployment_group_params, product: product, user: user} do
      changeset =
        deployment_group_params
