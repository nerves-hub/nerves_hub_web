defmodule NervesHub.DevicesTest do
  ...
  describe "update_network_interface/2" do
    ...
    test "doesn't allow invalid values", %{device: device} do
      ...
      assert cs.errors[:network_interface] == {"is not a valid network interface", []}

      {:error, _} = Devices.update_network_interface(device, "")
      {:error, _} = Devices.update_network_interface(device, nil)
    end
  end
end
