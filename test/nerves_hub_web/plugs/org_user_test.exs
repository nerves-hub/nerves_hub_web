defmodule NervesHubWeb.Plugs.OrgUserTest do
  use NervesHubWeb.ConnCase.Browser, async: true

  import Plug.Test

  alias NervesHub.Accounts
  alias NervesHub.Fixtures
  alias NervesHubWeb.Plugs.OrgUser

  setup context do
    {:ok, org_user} = Accounts.get_org_user(context.org, context.user)
    Map.put(context, :org_user, org_user)
  end

  test "org_user is set when found", %{conn: conn, org: org, org_user: org_user} do
    conn =
      conn
      |> Map.put(:assigns, %{org: org})
      |> Map.put(:params, %{"user_id" => org_user.user_id})
      |> OrgUser.call([])

    assert conn.assigns.org_user == org_user
  end

  test "404 is rendered user is not found", %{conn: conn, org: org} do
    conn =
      conn
      |> Map.put(:assigns, %{org: org})
      |> Map.put(:params, %{"user_id" => 000})
      |> OrgUser.call([])

    assert html_response(conn, 404) =~ "Sorry, the page you are looking can't be found."
  end

  test "404 is rendered when org_user is not found", %{conn: conn, org: org} do
    user2 = Fixtures.user_fixture()

    conn =
      conn
      |> Map.put(:assigns, %{org: org})
      |> Map.put(:params, %{"user_id" => user2.id})
      |> OrgUser.call([])

    assert html_response(conn, 404) =~ "Sorry, the page you are looking can't be found."
  end
end
