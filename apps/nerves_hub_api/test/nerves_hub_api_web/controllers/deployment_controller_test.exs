defmodule NervesHubAPIWeb.DeploymentControllerTest do
  ...
  describe "update deployment" do
    ...
    test "renders deployment when data is valid", %{conn: conn, deployment: deployment, product: product} do
      qp = URI.encode_query(%{product_name: product.name})
    ...
    test "renders errors when data is invalid", %{conn: conn, deployment: deployment, product: product} do
      qp = URI.encode_query(%{product_name: product.name})
      path = deployment_path(conn, :update, deployment.name) <> "?" <> qp
  ...
  defp create_deployment(%{tenant: tenant, product: product}) do
