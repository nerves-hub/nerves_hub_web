defmodule NervesHubAPIWeb.FirmwareController do
  ...
  def create(%{assigns: %{tenant: tenant}} = conn, _params) do
    with {:ok, filepath, conn} <- read_firmware(conn),
    {:ok, firmware_params} <- Firmwares.prepare_firmware_params(tenant, filepath),
  ...
  def show(%{assigns: %{tenant: tenant}} = conn, %{"uuid" => uuid}) do
    with {:ok, firmware} <- Firmwares.get_firmware_by_uuid(tenant, uuid) do
      render(conn, "show.json", firmware: firmware)
    end
  end

  def delete(%{assigns: %{tenant: tenant}} = conn, %{"uuid" => uuid}) do
    with {:ok, firmware} <- Firmwares.get_firmware_by_uuid(tenant, uuid),
