...
{success, errors} =
  ...
  |> Enum.reduce({[], []}, fn firmware, {success, errors} ->
    Firmware.changeset(firmware, %{org_id: firmware.org_key.org_id})
    |> Repo.update()
    |> case do
      {:ok, firmware} ->
...
{success, errors} =
  from(al in AuditLog, where: is_nil(al.org_id), preload: [:resource])
  |> Repo.all()
  |> Enum.reduce({[], []}, fn
      %{resource: %{org_id: org_id}} = audit_log, {success, errors} ->
