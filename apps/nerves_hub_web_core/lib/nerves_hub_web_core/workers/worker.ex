defmodule NervesHubWebCore.Worker do
  @moduledoc """
  ...
  * All other `Oban.Job.option()` are supported
  ...
  config:
  ```
  config :nerves_hub_web_core, :enable_workers, true
  ```
  ...
  defmacro __using__(config) do
    ...
    quote location: :keep do
      @behaviour NervesHubWebCore.Worker
  ...
  defp format_schedule!(config) do
    schedule =
      case Keyword.get(config, :schedule) do
        ...
        _ ->
          raise ArgumentError,
                "NervesHubWebCore.Worker use must have a valid cron expression.\n\t\"*/5 * * * *\"\n\t~e[*/5 * * * *]"
      end
      |> Macro.escape()
