defmodule NervesHubWebCore.Stats do
  @moduledoc """
  A module for creating and sending StatsD metrics
  """
  use GenServer

  alias NervesHubWebCore.Statix, as: ServerStatix
  ...
  def init(_opts) do
    ...
    host = Application.get_env(:nerves_hub_web_core, :statsd_host, nil)
    port = Application.get_env(:nerves_hub_web_core, :statsd_port, nil)
  ...
  def handle_call({:measure, params}, _from, :connected = state) do
    {:reply, apply(NervesHubWebCore.Statix, :measure, params), state}
  end
  ...
  def handle_cast({method, params}, :connected = state) do
    apply(NervesHubWebCore.Statix, method, params)
    {:noreply, state}
  end

  def options(additional_tags) do
    env = Application.get_env(:nerves_hub_web_core, :env)
  ...
  ## Examples
      iex> base_options = [prefix: "nerves_hub_web_core.", tags: ["env:dev", "service:nerves_hub_web_core"]]
      iex> additional = [{:sample_rate, 1.0}, tags: ["http_status:200", "http_host:smarthome-cahatlas-qa.herokuapp.com", "http_status_family:2xx"]]
      iex> NervesHubWebCore.Stats.handle_additional_tags(additional, base_options)
      [
        tags: ["http_status:200", "http_host:smarthome-cahatlas-qa.herokuapp.com",
        "http_status_family:2xx", "env:dev", "service:nerves_hub_web_core"],
        sample_rate: 1.0,
        prefix: "nerves_hub_web_core."
      ]
      iex> base_options = [prefix: "nerves_hub_web_core.", tags: ["env:dev", "service:nerves_hub_web_core"]]
      iex> additional = []
      iex> NervesHubWebCore.Stats.handle_additional_tags(additional, base_options)
      [prefix: "nerves_hub_web_core.", tags: ["env:dev", "service:nerves_hub_web_core"]]
  """
  ...
  defp dyno_type do
    Application.get_env(:nerves_hub_web_core, :dyno, "web")
  end
end

defmodule NervesHubWebCore.Statix do
  use Statix
end
