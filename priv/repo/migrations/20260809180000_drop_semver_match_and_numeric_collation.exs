defmodule NervesHub.Repo.Migrations.DropSemverMatchAndNumericCollation do
  use Ecto.Migration

  # `semver_sort_key/1` (see add_semver_sort_key_function) now backs all firmware
  # version ordering and comparison across the app. Two older mechanisms it
  # replaced are no longer referenced anywhere and are removed here:
  #
  #   * the hand-rolled `semver_match(text, text)` plpgsql (and its
  #     `compare_prerelease`/`compare_prerelease_ids` helpers) — previously used
  #     for the priority-queue version threshold WHERE, now rewritten to
  #     `semver_sort_key`;
  #   * the ICU `numeric` collation — previously used for the `ORDER BY`s in
  #     NervesHub.Firmwares, now rewritten to `semver_sort_key(...) COLLATE "C"`.
  #
  # `down/0` recreates them exactly as they were (the current definitions from
  # the create_numeric_collation and add_prerelease_support_to_semver_match
  # migrations) so the migration is fully reversible.
  def up() do
    execute("DROP FUNCTION IF EXISTS semver_match(text, text);")
    execute("DROP FUNCTION IF EXISTS compare_prerelease(text, text);")
    execute("DROP FUNCTION IF EXISTS compare_prerelease_ids(text, text);")
    execute("DROP COLLATION IF EXISTS numeric;")
  end

  def down() do
    execute("CREATE COLLATION IF NOT EXISTS numeric (provider = icu, locale = 'en-u-kn-true');")
